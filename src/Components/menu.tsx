					...
					{filteredItems.length > 0 ? (
						filteredItems.map((item) => (
							...
							>
								...
								/>
								<div className="p-3">
									<h3 className="text-sm text-[#138F96] font-bold text-right">
										{item.title}
									</h3>
...
//   // محاسبه totalPrice با useMemo
//   const totalPrice = useMemo(() => {
//     console.log("Calculating totalPrice, cart:", cart);
//     const price = Object.entries(cart).reduce((sum, [itemId, quantity]) => {
//       const item = allMenuItems.find((i) => i.id === parseInt(itemId));
//       console.log(`Item ${itemId}:`, { item, quantity, fee: item?.fee });
//       return sum + (item ? item.fee * quantity : 0);
//     }, 0);
...
//   const updateQuantity = (itemId: number, delta: number) => {
//     setCart((prev) => {
//       console.log(`Updating quantity for item ${itemId}, delta: ${delta}`);
//       const newQuantity = (prev[itemId] || 0) + delta;
//       if (newQuantity <= 0) {
//         const { [itemId]: _, ...rest } = prev;
//         console.log(`Removed item ${itemId} from cart`);
...
//         items = response.data.items || response.data.data || [];
//       }
//       console.log("Menu Items API Response:", items);
//       setMenuItems(items);
//     } catch (error) {
//       console.error("Error fetching menu items:", error);
//       setMenuItems([]);
...
//   // اعتبارسنجی allMenuItems
//   useEffect(() => {
//     console.log("allMenuItems:", allMenuItems);
//     if (allMenuItems.length === 0) {
//       console.warn("allMenuItems is empty!");
//     }
//   }, [allMenuItems]);
...
//         <div className="text-right">
//           <p className="text-sm text-[#1B1D1D]">مجموع سفارش</p>
//           <p className="text-sm font-bold">{totalPrice.toLocaleString()} تومان</p>
//         </div>
//       </div>
//     </div>
//   );
